...
my_profile = pd.DataFrame({"Group": ["My profile"], "Profile coherence": [90], "Potential contribution": [95]})
comparison_df = pd.concat([comparison_df, my_profile], ignore_index=True)

chi_table = pd.DataFrame({
    "Profile dimension": ["Analytical skills", "Applied economics", "Writing", "Creativity", "Synthesis", "Data analysis", "French and English", "Project management"],
    "Present / aligned": [20, 18, 16, 15, 14, 17, 20, 19],
    "Gap / not aligned": [0, 0, 0, 0, 0, 0, 0, 0]
})
chi_for_calc = chi_table[["Present / aligned", "Gap / not aligned"]].replace(0, 0.5)
chi2, p, dof, expected = chi2_contingency(chi_for_calc)

timeline = pd.DataFrame({
    "Activity": [
        "Master in Development and Applied Economics",
        "Freelance research and empirical studies",
        "Statistics officer / data-oriented professional role",
        "Part-time doctoral researcher",
    ],
    "Start": [2020, 2020, 2021, 2022],
    "End": [2020.3, 2025, 2024, 2025],
})
...
st.markdown(
    """
This dashboard presents my professional profile as a **model**, not simply as a translated document and not as an application focused only on one job title.
...
The logic is simple: a professional profile is not built from one element only. It is the result of accumulated education, practical experience, analytical thinking, technical capacity, communication, adaptability, and the ability to transform knowledge into measurable contribution.
"""
    )
...
"""
    ...
    fig = px.scatter(
        ...
        color="Group",
        symbol="Group",
        size=np.where(comparison_df["Group"].eq("My profile"), 16, 8),
    ...
    st.markdown(
        """
The original document includes a chi-square-style test. In this dashboard, it is presented as a profile-coherence test between the main competencies and the overall professional positioning.
"""
    )
    ...
    st.markdown("This timeline summarizes the accumulation of academic, research, and professional experience that supports the profile model.")
    fig = px.timeline(
        timeline,
        x_start="Start",
        x_end="End",
        y="Activity",
        title="Academic and professional trajectory",
    )
